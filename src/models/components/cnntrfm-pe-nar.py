...
class CNNTRFM_PE2_NAR(nn.Module):
    def __init__(
        self,
        kernel_sizes: List = [6, 9, 12, 15],
        out_channels: int = 256,
    ...
    def forward(self, x, init_level, y):
        # x: (N, L, C)
        ...
        tgt = torch.cat([init_level.unsqueeze(-1), y], axis=1)[:, :-1]  # (N, L)
        tgt = tgt.unsqueeze(-1)  # (N, L, 1)
        tgt = self.embed_tgt(tgt)  # (N, L, d_model)
        tgt = tgt.transpose(0, 1)  # (L, N, d_model)
        tgt_mask = nn.Transformer.generate_square_subsequent_mask(8).to(x.device)
        
        out = self.decoder(tgt, x, tgt_mask=tgt_mask)  # (L, N, d_model)
        out = self.out(out.transpose(0, 1))  # (N, 8)
        
        return out.squeeze(-1)
        
    def infer(self, x, init_level):
        # x: (N, L, C)
        x = self.encoder(x)  # (N, C, L)
        x = x.transpose(1, 2)  # (N, L, C)
        x = self.fc1(x)  # (N, L, d_model)
